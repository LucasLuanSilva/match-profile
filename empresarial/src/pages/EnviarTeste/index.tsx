...
const EnviarTeste: React.FC = () => {

  const navigation = useNavigation();
  const route = useRoute();
  const [load, setLoad] = useState(true);

  const [teste] = useState(route.params.teste);
  ...
  const enviarTeste = async () => {
    ...
    await api.post('empresariais/testes_atribuidos', body).then((response) => {
      Alert.alert("Envio realizado com sucesso !");
      navigation.goBack();
    }).catch((error) => {
      toggleModal();
